...
export default function TimesheetRow({ row, onChange, onRowUpdate, isSaving }: TimesheetRowProps) {
  ...
  function handleActivityChange(value: string) {
    ...
    if (autofill) {
      ...
      onRowUpdate(row.date, { activity: '', start_time: '', end_time: '', remarks: '' })
    }
  }
  const isInvalidTime = row.start_time && row.end_time && row.end_time <= row.start_time
  ...
  return (
    ...
    `}>
      ...
      <td className="px-2 py-1.5 w-32">
        ...
        />
          {isInvalidTime && (
            <p className="text-red-400 text-[10px] mt-0.5">End time harus lebih besar</p>
          )}
      </td>
